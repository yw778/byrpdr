 ...
 *
 */
public class MainActivity111 extends Activity implements SensorEventListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		for(int i=0;i<=j-2;i++) afterLPF[i]=0;
	}
...
//		float roll = mOrientation[2];
		...
		if (t>=1) {
			...
			hGro += (-(float)Math.toDegrees(g_GCS_z*(time[t]-time[t-1])));//陀螺仪时间和累加
		...
		if(t>=j+N-1) //N?
		{	
			if(findPeak1(afterLPF,t)==1) count1++;//由条件1得到的步数
			if(ensurePP(afterLPF)==1) count2++;//由条件2得到的步数
			if(ensureSlope(afterLPF)==1) count3++;//由条件3得到的步数
			text_mR.setText("step is:"+step+"\n"+count1+"\n"+count2+"\n"+count3);
			if((t-pre_add)>7)
				can_add=true;
			if(findPeak1(afterLPF,t)*ensurePP(afterLPF)*ensureSlope(afterLPF)==1){
				if(can_add){
					step++;
...
//		text_mR.setText(display(mR1));
	...
	private int findPeak1(float[] afterLPF, int t2) {
		int isPeak=1;
		for(int i=0;i<=N;i++){
			if(afterLPF[t2-N/2]<afterLPF[t2-i])
				isPeak=0;
			if(afterLPF[t2-N/2]<0.45)
				isPeak=0;
		}
	...
	private int ensurePP(float[] afterLPF)	
	{		
		float[] minus_previous = new float[j];
		float[] minus_latter = new float[j];
		
		for(int i=0;i<N/2;i++){
			minus_previous[i]=afterLPF[t-N/2]-afterLPF[t-N/2-i-1];
			minus_latter[i]=afterLPF[t-N/2]-afterLPF[t-N/2+i+1];
		}
	...
	private int ensureSlope(float[] afterLPF){
		int isPeak=1;
//		float sum_previous = 0, sum_latter=0;
		float previous=0,latter=0;
		for(int i=t-N;i<=t-N/2-1;i++){
			previous = afterLPF[i+1] - afterLPF[i];
			latter = afterLPF[i+N/2+1] - afterLPF[i+N/2];
...
//		else 
//			stepLength=(float) (1.479*Math.pow(apk2vly,0.25)-1.123);
//		
//		return  stepLength;
//		
//	}
	...
	private float calcuDis(){
		...
		/*if(findPeak1(getNegetive(afterLPF),i)*ensurePP(getNegetive(afterLPF),i)*ensureSlope(getNegetive(afterLPF),i)==1)*/	
		break;
		}
		apk2vly=afterLPF[t-N/2]-afterLPF[t-N/2-i];
		if(apk2vly>=a_thresh)
			stepLength=(float) (1.131*Math.log10(apk2vly)+0.159);
		else 
